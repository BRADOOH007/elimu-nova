...
function findMatchingBrace(text, startIdx) {
  ...
  while (i < text.length && depth > 0) {
    ...
    if (inS) { if (ch === "'" && prev !== '\\') inS = false }
      ...
      else if (ch === '$' && text[i + 1] === '{') { const c = findMatchingBrace(text, i + 1); if (c === -1) return -1; i = c }
    } else {
...
function getScopeName(filePath) {
  const rel = filePath.replace(/\\/g, '/').replace(/.*src\/app\/api\//, '').replace(/\/route\.ts$/, '')
...
function transformContent(content, filePath) {
  ...
  // If already has route({ and no getServerSession, just add import
  if (/route\s*\(\s*\{/.test(content)) {
    if (!content.includes('getServerSession')) {
      return addMiddlewareImport(content, scope)
    }
    // Has leftover getServerSession - use backup
    const bak = filePath + '.codemod.bak'
    const bakContent = fs.existsSync(bak) ? fs.readFileSync(bak, 'utf-8') : null
...
for (const filePath of brokenFiles) {
  ...
  console.log(`\n--- ${filePath} ---`)
  console.log(`Original size: ${content.length}, has route(: ${/route\s*\(\s*\{/.test(content)}, has session: ${content.includes('getServerSession')}`)
  ...
  if (transformed && transformed !== content) {
    ...
    console.log(`✅ Converted! Size: ${content.length} -> ${newSize}`)
  } else if (transformed === null) {
    console.log(`⚠️  No handlers found`)
  } else {
    console.log(`⚠️  No change (transformed === content)`)
  }
}
