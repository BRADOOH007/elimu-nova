...
import { X, Minimize2, Maximize2, FileText, ChevronUp } from 'lucide-react'
...
export function FloatingNotesWidget({ notes, setNotes, savedNotes, saveNote, activeLesson }: FloatingNotesWidgetProps) {
  ...
  const toggleExpand = () => setMode(m => m === 'expanded' ? 'minimized' : 'expanded')
    ...
    return (
      <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-2">
        <button onClick={() => setMode('expanded')}
          className="flex items-center gap-2 rounded-full bg-gradient-to-r from-purple-600 to-indigo-600 px-4 py-2.5 text-sm font-bold text-white shadow-xl transition-all hover:scale-105 hover:shadow-2xl">
    ...
    return (
      <div className="fixed bottom-6 right-6 z-50 flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 shadow-xl backdrop-blur">
        ...
        <div className="ml-2 flex items-center gap-1">
          <button onClick={toggleExpand} className="rounded-full p-1 text-slate-400 hover:bg-slate-100 hover:text-purple-600" title="Restore"><Maximize2 className="h-4 w-4" /></button>
          <button onClick={() => setMode('closed')} className="rounded-full p-1 text-slate-400 hover:bg-slate-100 hover:text-red-500" title="Close"><X className="h-4 w-4" /></button>
        </div>
      </div>
    )
  }
    ...
    <div className="fixed bottom-6 right-6 z-50 w-96 rounded-2xl border border-slate-200 bg-white shadow-2xl backdrop-blur">
      ...
      <div className="flex items-center justify-between rounded-t-2xl border-b border-slate-100 bg-gradient-to-r from-purple-50 to-indigo-50 px-4 py-3">
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => setMode('minimized')} className="rounded-full p-1 text-slate-400 hover:bg-slate-100 hover:text-amber-600" title="Minimize"><Minimize2 className="h-4 w-4" /></button>
          <button onClick={() => setMode('closed')} className="rounded-full p-1 text-slate-400 hover:bg-slate-100 hover:text-red-500" title="Close"><X className="h-4 w-4" /></button>
        </div>
      </div>
