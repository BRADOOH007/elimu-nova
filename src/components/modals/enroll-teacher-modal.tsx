...
function PasswordDisplay({ password }: { password: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="font-mono font-semibold">{show ? password : '\u2022'.repeat(8)}</span>
...
function Pill({ active, onClick, children, badge }: { active: boolean; onClick: () => void; children: React.ReactNode; badge?: string }) {
  return (
    ...
    >
      ...
      {badge && (
        <span className={cn(
          "text-[9px] font-bold uppercase tracking-wide rounded-full px-1.5 py-0.5 leading-none",
...
export function EnrollTeacherModal({ isOpen, onClose, onSuccess }: EnrollTeacherModalProps) {
  ...
  useEffect(() => {
    if (isOpen && !generatedPassword) setGeneratedPassword(generatePassword())
  }, [isOpen])

  useEffect(() => {
    if (isOpen && !credentials) {
      saveDraft({ formData, generatedPassword, selectedGrades, manualSubjects, removedAutoSubjects })
    }
  }, [formData, generatedPassword, selectedGrades, manualSubjects, removedAutoSubjects, isOpen, credentials])
  ...
  const autoAssignedSubjects = useMemo(() => {
    ...
    for (const grade of selectedGrades) {
      for (const subject of CORE_SUBJECTS_BY_GRADE[grade] || []) set.add(subject)
    }
    return [...set]
  }, [selectedGrades])
  ...
  const optionalSubjects = useMemo(
    () => ALL_SUBJECTS.filter(s => !autoAssignedSubjects.includes(s)),
  ...
  const toggleSubject = (subject: string) => {
    ...
    if (isSelected) {
      setManualSubjects(prev => prev.filter(s => s !== subject))
      if (isAuto) setRemovedAutoSubjects(prev => [...prev, subject])
    } else {
      if (isAuto) setRemovedAutoSubjects(prev => prev.filter(s => s !== subject))
      else setManualSubjects(prev => (prev.includes(subject) ? prev : [...prev, subject]))
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        clearDraft()
        setCredentials({
          username: data.teacher?.username || previewUser,
          password: data.teacher?.password || generatedPassword,
          email: formData.email,
      ...
      {credentials ? (
        <div className="space-y-4">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
            ...
            {credentials.password && (
              <div><p className="text-xs text-slate-500">Password</p><PasswordDisplay password={credentials.password} /></div>
            )}
          </div>
          <p className="text-xs text-amber-600 text-center">Save these credentials now — the password cannot be recovered later</p>
          <div className="flex gap-2">
            <button onClick={copyCredentials} className="flex-1 rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 transition inline-flex items-center justify-center gap-1.5">
              <Copy className="h-4 w-4" />Copy All
            </button>
            <button onClick={handleDone} className="flex-1 rounded-lg bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 px-5 py-2 text-sm font-medium text-white shadow-sm transition">
              Done
            </button>
          </div>
        </div>
        ...
        <form id="enroll-teacher-form" onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <AdminFormField label="First Name" htmlFor="enroll-first" required>
              <input id="enroll-first" type="text" autoComplete="off" placeholder="Enter first name"
                value={formData.firstName} onChange={e => setFormData(prev => ({ ...prev, firstName: e.target.value }))}
                className={adminInputClass} required />
            </AdminFormField>
            <AdminFormField label="Last Name" htmlFor="enroll-last" required>
              <input id="enroll-last" type="text" autoComplete="off" placeholder="Enter last name"
          ...
          {(formData.firstName || formData.lastName) && (
            <div className="bg-gradient-to-br from-indigo-50 to-violet-50 border border-indigo-100 rounded-xl p-3 space-y-1.5">
              <p className="text-xs font-semibold text-indigo-700 flex items-center gap-1">
                <Sparkles className="w-3 h-3" /> Auto-generated Credentials
              </p>
              ...
              <div className="flex items-center gap-2 text-xs">
                ...
                <code className="font-mono bg-white px-2 py-0.5 rounded text-slate-800">
                  {showPassword ? generatedPassword : '\u2022'.repeat(generatedPassword.length)}
                </code>
                <button type="button" onClick={() => setShowPassword(v => !v)} className="text-slate-400 hover:text-slate-600" title={showPassword ? 'Hide' : 'Show'}>
                  {showPassword ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                </button>
                <button type="button" onClick={copyPassword} className="text-slate-400 hover:text-slate-600" title="Copy password">
                  <Copy className="h-3.5 w-3.5" />
                </button>
                <button type="button" onClick={() => setGeneratedPassword(generatePassword())} className="text-slate-400 hover:text-indigo-600 ml-auto inline-flex items-center gap-1" title="Regenerate">
          ...
          <AdminFormField label="Email Address" htmlFor="enroll-email" required>
            <input id="enroll-email" type="email" autoComplete="off" placeholder="Enter email address"
              value={formData.email} onChange={e => setFormData(prev => ({ ...prev, email: e.target.value }))}
              className={adminInputClass} required />
          </AdminFormField>
          <AdminFormField label="Phone Number" htmlFor="enroll-phone">
            <input id="enroll-phone" type="tel" autoComplete="off" placeholder="Enter phone number"
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-700">Grade Levels</span>
            </div>
            <div className="space-y-3">
                ...
                <div key={band.label}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {band.grades.map(grade => (
                      <Pill key={grade} active={selectedGrades.includes(grade)} onClick={() => toggleGrade(grade)}>
                        {grade}
                      </Pill>
                    ))}
                  </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-700">Learning Areas / Subjects</span>
            </div>
            <div className="flex flex-wrap items-center gap-1.5">
              <button type="button" onClick={selectOnlyCore}
                className="text-[11px] font-medium text-indigo-600 bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 rounded-full px-2.5 py-1 transition-colors">Core subjects</button>
              <button type="button" onClick={selectAllSubjects}
                className="text-[11px] font-medium text-slate-600 bg-slate-50 border border-slate-200 hover:bg-slate-100 rounded-full px-2.5 py-1 transition-colors">Select all</button>
              <button type="button" onClick={clearSubjects}
                className="text-[11px] font-medium text-rose-500 bg-rose-50 border border-rose-100 hover:bg-rose-100 rounded-full px-2.5 py-1 transition-colors">Clear</button>
            ...
            {selectedGrades.length > 0 && (
              <p className="text-xs text-indigo-600 bg-indigo-50 border border-indigo-100 rounded-lg px-3 py-2">
                Core subjects for the selected grades are pre-selected. Tap any pill to adjust.
              </p>
            )}
            {autoAssignedSubjects.length > 0 && (
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-2">
                  {autoAssignedSubjects.map(subject => (
                    <Pill key={subject} active={effectiveSubjects.includes(subject)} onClick={() => toggleSubject(subject)} badge="Core">
                      {subject}
                    </Pill>
                  ))}
                </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-2">
                  {optionalSubjects.map(subject => (
                    <Pill key={subject} active={effectiveSubjects.includes(subject)} onClick={() => toggleSubject(subject)}>
                      {subject}
                    </Pill>
                  ))}
                </div>
