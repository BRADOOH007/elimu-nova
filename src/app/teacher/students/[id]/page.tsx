...
import { Loader2, ArrowLeft, User, Mail, Phone, MapPin, School, Calendar, BookOpen, CheckCircle, Clock, AlertTriangle, Brain, TrendingUp, BarChart3 } from "lucide-react"
      ...
      <Tabs defaultValue="assignments" className="space-y-4">
        ...
        <TabsContent value="progress">
          <Card className="border-0 shadow">
            <CardContent className="p-6">
              {!progress ? (
                <p className="text-center text-gray-500">No progress data</p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <h3 className="font-semibold flex items-center gap-2"><TrendingUp className="w-4 h-4 text-blue-600" /> Performance</h3>
                    <div className="space-y-3">
                      <div><div className="flex justify-between text-sm"><span>Average Grade</span><span className="font-medium">{progress.averageGrade !== null ? `${Math.round(progress.averageGrade)}%` : 'N/A'}</span></div><div className="w-full bg-gray-200 rounded-full h-2 mt-1"><div className="bg-blue-600 h-2 rounded-full" style={{ width: `${progress.averageGrade || 0}%` }} /></div></div>
                      <div><div className="flex justify-between text-sm"><span>Completed</span><span className="font-medium">{progress.completedAssignments}</span></div></div>
                      <div><div className="flex justify-between text-sm"><span>Pending</span><span className="font-medium">{progress.pendingAssignments}</span></div></div>
                      <div><div className="flex justify-between text-sm"><span>Overdue</span><span className="font-medium text-red-600">{progress.overdueAssignments}</span></div></div>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <h3 className="font-semibold flex items-center gap-2"><Clock className="w-4 h-4 text-green-600" /> Engagement</h3>
                    <div className="space-y-3">
                      <div><div className="flex justify-between text-sm"><span>Total Study Time</span><span className="font-medium">{Math.round(progress.totalStudyTime / 60)} hours</span></div></div>
                      <div><div className="flex justify-between text-sm"><span>Current Streak</span><span className="font-medium">{progress.streakDays} days</span></div></div>
                      <div><div className="flex justify-between text-sm"><span>Last Active</span><span className="font-medium">{progress.lastActiveDate ? new Date(progress.lastActiveDate).toLocaleDateString() : 'Never'}</span></div></div>
                    </div>
                  </div>
                </div>
              )}
