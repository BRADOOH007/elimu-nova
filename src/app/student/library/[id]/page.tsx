...
import {
  ...
  Star, Minus, Plus, Moon, Sun, CheckCircle, Download, MessageCircle, Sparkles, WifiOff, Clock
} from 'lucide-react'
...
const cacheKey = (id: string) => `book_content_${id}`
...
export default function BookReaderPage() {
  ...
  if (error || !book) {
    return (
      <div className="max-w-2xl mx-auto p-6 text-center">
        ...
        <Link href="/student/library" className="text-blue-600 hover:underline text-sm">Back to library</Link>
      </div>
    )
  }
  ...
  return (
    <div className={`min-h-screen ${nightMode ? 'bg-slate-950' : 'bg-slate-50'} transition-colors`}>
      <div className="sticky top-0 z-20 border-b bg-white/90 backdrop-blur">
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
          <Link href="/student/library" className="flex items-center gap-1.5 text-sm text-slate-600 hover:text-slate-900">
            <ArrowLeft className="h-4 w-4" /> Library
          </Link>
          <p className="text-sm font-semibold text-slate-800 truncate flex-1 text-center">{book.title}</p>
          <div className="flex items-center gap-2">
            <button onClick={() => setFontSize(f => Math.max(14, f - 1))} className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100"><Minus className="h-4 w-4" /></button>
            <button onClick={() => setFontSize(f => Math.min(28, f + 1))} className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100"><Plus className="h-4 w-4" /></button>
            <button onClick={() => setNightMode(v => !v)} className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100">
            ...
            <div className={`flex items-center justify-between flex-wrap gap-3 ${nightMode ? 'text-slate-400' : 'text-slate-500'}`}>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => runCompanion('quiz')}
                  className="flex items-center gap-1.5 text-sm font-medium text-slate-600 hover:text-blue-600 px-2 py-1 rounded-lg hover:bg-blue-50">
        ...
        {related.length > 0 && (
          <div className={`mt-6 rounded-2xl border p-4 ${nightMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
            ...
            <div className="flex gap-4 overflow-x-auto pb-1">
              {related.map(b => (
                <Link key={b.id} href={`/student/library/${b.id}`} className="shrink-0 w-32">
        ...
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-30">
          <div className={`flex items-center gap-3 px-4 py-2.5 rounded-full shadow-lg border ${nightMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
            ...
            <button onClick={tts.toggle}
              className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-600 to-purple-600 text-white flex items-center justify-center shadow">
