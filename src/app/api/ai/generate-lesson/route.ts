...
export const POST = route({ auth: ['TEACHER', 'SCHOOL_ADMIN', 'SUPER_ADMIN'] }, async (req, { user }) => {
    ...
    const content = await OpenAIService.generateText([
      { role: 'system', content: `You are an expert AI tutor creating unique lesson content for students. Use a ${approach} approach — vary examples and structure every time. Return ONLY valid JSON with: title, subject, grade, difficulty, duration, type, objectives (array), prerequisites (array), insights (array), content (detailed markdown lesson body).` },
      { role: 'user',   content: `Create a unique, personalized lesson for: Subject: ${subject}, Topic: ${topic}, Grade: ${grade || 'Grade 8'}, Level: ${difficulty || 'intermediate'}, Style: ${learningStyle || 'visual'}. Request ID: ${requestId}. Use a specific, relatable local example that's different from any previous lesson. Return ONLY valid JSON.` },
